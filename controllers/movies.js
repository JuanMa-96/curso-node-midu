...
export class MovieController {
  ...
  getById = async (req, res) => {
    const { id } = req.params
    const movie = await this.movieModel.getById({ id }) // <-- Es equivalente a pasar req.params (extrae el id con la destructuracion {})
  ...
  update = async (req, res) => {
    ...
    if (updatedMovie) res.status(200).json(updatedMovie)
  }
}
